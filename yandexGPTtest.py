...
# Чтение данных из authorized_key.json
def load_authorized_key(file_path):
    with open(file_path, 'r') as f:
        data = json.load(f)
    return data

# Формирование JWT
def create_jwt_token(service_account_id, key_id, private_key):
    now = int(time.time())
    payload = {
...
def get_iam_token(auth_data):
    jwt_token = create_jwt_token(
        service_account_id=auth_data['service_account_id'],
        key_id=auth_data['id'],
    ...
    if response.status_code == 200:
        ...
        print("Ошибка получения токена:", response_data)
        return None


# Использование IAM-токена для запроса
def text_to_yandexGPT_summarize(text_incoming: str, auth_file_path: str) -> str:
    auth_data = load_authorized_key(auth_file_path)
    iam_token = get_iam_token(auth_data)
    print("Получен IAM-токен:", iam_token)
    if not iam_token:
        return "Не удалось получить IAM-токен"

    modelUri = get_modelUri_yandexGPT()  # Ваша функция для получения URI модели
    prompt = {
        "modelUri": f'{modelUri}',
        "completionOptions": {
            "stream": False,
        ...
        "messages": [
            {
                ...
                "text": "Шаг 1: Определение принадлежности фраз "
                        "Определи, кому принадлежит каждая фраза: менеджеру по продажам или клиенту. Для этого обозначь начало каждой реплики соответствующими метками: Менеджер и Клиент. Менеджера зовут Евгений. Он из компании ЗЕВТЕК. Распознавание разговора может быть с ошибками, поэтому если фраза похожа исходи из вышесказанного."
                        ...
                        "Не выводи в ответе результат шагов 1,2, только 3 как можно подробнее, на основе этого будет вестись дальнейшая работа с клиентом. Не придумывай ничего - тебя проверят. Ответ должен содержать не строго не более 4000 символов."
            },
            {
                "role": "user",
    ...
    if 'result' in result:
        ...
        print("Ошибка в запросе к GPT:", result)
        return None

# Пример использования
#print(text_to_yandexGPT_summarize("Ваш текст здесь", auth_file_path))
